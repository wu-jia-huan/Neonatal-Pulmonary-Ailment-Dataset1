...
from cutmix.cutmix import cutmix_pairwise
...
def train(args, snapshot_path):
    ...
    model1 = net_factory(net_type=args.model, in_chns=1, class_num=num_classes)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            output1 = model1(mixed_data)
            outputs_soft1 = torch.softmax(output1, dim=1)
            output2 = model2(mixed_data)
            outputs_soft2 = torch.softmax(output2, dim=1)
            consistency_weight = get_current_consistency_weight(iter_num // 150)
            loss_sur = criteria(output1, mixed_target_a) * lam_mix + criteria(output2, mixed_target_b) * (1. - lam_mix)
            ...
            pseudo_supervision1 = criteria(output1[args.labeled_bs:], pseudo_outputs2)
            pseudo_supervision2 = criteria(output2[args.labeled_bs:], pseudo_outputs1)

            model1_loss =  consistency_weight * pseudo_supervision1 
            model2_loss =  consistency_weight * pseudo_supervision2 
            loss = model1_loss + model2_loss+loss_sur
