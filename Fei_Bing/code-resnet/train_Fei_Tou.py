...
from dataloaders.dataset_yrh import BaseDataSets, RandomGenerator, BaseDataSets_xueyoubing, RandomGenerator_single, BaseDataSets_Xi_Ru, BaseDataSets_Fei_Tou,BaseDataSets_Fei_Tou_txt
...
parser.add_argument('--text_file',type=str,default='/mnt/sdd/wjh/Fei_Bing/dataset_text/Fei_Tou.csv',help='text-file')
...
def discretize_gestational_age(gestation_str):
    ...
    if '+' in week_str:
        ...
        if week_str.isdigit():
            ...
            weeks = 25  # 如果格式不正确，返回默认值
    index = weeks - 25
    return max(0, min(index, 20))  # 假设25周是最小值
...
# 数值数据的Embedding
age_embedding = nn.Embedding(num_embeddings=24, embedding_dim=embedding_dim)  # 最大的24天
gestational_age_embedding = nn.Embedding(num_embeddings=21, embedding_dim=embedding_dim)  # 假设25周到45周
...
def vectorize_text_info(text_info):
    ...
    # 去掉 '是否窒息' 列中的 '无'
    asphyxia_list = [asphyxia if asphyxia != '无' else '否' for asphyxia in text_info['是否窒息史']]
    ...
    birth_weight_idx = torch.tensor([discretize_birth_weight(weight) for weight in text_info['体重']])
...
def train(args, snapshot_path):
    ...
    db_train = BaseDataSets_Fei_Tou_txt(base_dir=args.root_path, split="train", text_file=args.text_file,num=None, transform=transforms.Compose([
        RandomGenerator_single(args.patch_size)
    ]))
    db_val = BaseDataSets_Fei_Tou_txt(base_dir=args.root_path, split="val",text_file=args.text_file)
    #yrh
    db_test = BaseDataSets_Fei_Tou_txt(base_dir=args.root_path, split="test",text_file=args.text_file)
    ...
    model_path_best = "/mnt/sdd/wjh/Fei_Bing/model/cross_pesudo_fei_tou2/Fully_DenseNet50_num_140_labeled_Acc_.710_1/iter_30000.pth"
