...
class unet_nodrop(nn.Module):
    def __init__(self, in_chns, class_num):
        super(unet_nodrop, self).__init__()
...
class unet_feature_away(nn.Module):
    def __init__(self, in_chns, class_num):
        super(unet_feature_away, self).__init__()
...
######################################################################1    
#unet_change_skip_concat 是为了在交换权重的时候关闭跳跃连而写的方法
class unet_change_skip_concat(nn.Module):
    def __init__(self, in_chns, class_num):
        super(unet_change_skip_concat, self).__init__()
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder_change_skip_concat(params)

    def forward(self, x, skip_concat):
        feature = self.encoder(x)
        output = self.decoder(feature, skip_concat)
        return output
class Decoder_change_skip_concat(nn.Module):
    def __init__(self, params):
        super(Decoder_change_skip_concat, self).__init__()
        self.params = params
        self.in_chns = self.params['in_chns']
        self.ft_chns = self.params['feature_chns']
        self.n_class = self.params['class_num']
    ...
    def forward(self, feature, skip_concat):
        if skip_concat:
            x0 = feature[0]
            x1 = feature[1]
            x2 = feature[2]
...
class DenseNet(nn.Module):
    ...
    """
    def __init__(self, in_chns=1, growth_rate=12, block_config=(16, 16, 16), compression=0.5,
        ...
        if small_inputs:
            self.features = nn.Sequential(OrderedDict([
                ('conv0', nn.Conv2d(in_chns, num_init_features, kernel_size=3, stride=1, padding=1, bias=False)),
            ]))
        else:
            self.features = nn.Sequential(OrderedDict([
                ('conv0', nn.Conv2d(in_chns, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
            ]))
        ...
        self.classifier_2 = nn.Linear(num_features, num_classes)
        self.classifier_3 = nn.Linear(num_features, num_classes)
        self.classifier_4 = nn.Linear(num_features, num_classes)
        self.classifier_5 = nn.Linear(num_features, num_classes)
    ...
    def forward(self, x):
        ...
        # print("out_1", out_1)
        """  tensor([[-0.1968, -0.1044, -0.1467, -0.0547, -0.0184,  0.2673],
        [-0.2152, -0.0957, -0.1727, -0.0562,  0.0036,  0.2620],
        [-0.2580, -0.0705, -0.1131, -0.0931,  0.0210,  0.3186],
        [-0.1868, -0.1100, -0.1466, -0.0800, -0.0289,  0.2119]], """ 
        out_2 = self.classifier_2(out)
        out_3 = self.classifier_3(out)
        out_4 = self.classifier_4(out)
        out_5 = self.classifier_5(out)
        return [out_1, out_2, out_3, out_4, out_5]


# 为了构建吸入综合征的，先做一个二分类的模型。
class DenseNet_2_class(nn.Module):
    r"""Densenet-BC model class, based on
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`
    Args:
        growth_rate (int) - how many filters to add each layer (`k` in paper)
    ...
    def __init__(self, in_chns=1, growth_rate=12, block_config=(16, 16, 16), compression=0.5,
        ...
        super(DenseNet_2_class, self).__init__()
        assert 0 < compression <= 1, 'compression of densenet should be between 0 and 1'
        ...
        self.classifier_combined = nn.Linear(num_features + 60, num_classes)
    ...
    def forward(self, x,text_features = None):
        ...
        out = torch.flatten(out, 1)
        # 如果有文本特征，则进行融合
        if text_features is not None and x.size(0) == 4:
            combined_features = torch.cat((out, text_features), dim=1)
            out_1= self.classifier_combined(combined_features)
