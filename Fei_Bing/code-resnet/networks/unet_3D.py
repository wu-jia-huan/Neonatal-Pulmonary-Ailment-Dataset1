...
class unet_3D(nn.Module):
    ...
    def apply_argmax_softmax(pred):
        ...
        return log_p
# Brats 的Cocoon 
class unet_3D_Cocoon(nn.Module):

    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3, is_batchnorm=True):
        super(unet_3D_Cocoon, self).__init__() 
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
    ...
    def forward(self, inputs, skip_concat):
        conv1 = self.conv1(inputs)
        maxpool1 = self.maxpool1(conv1)
        ...
        if skip_concat:
            ...
            up4 = self.up_concat4(conv4, center)
            up3 = self.up_concat3(conv3 * 0, up4)
            up2 = self.up_concat2(conv2 * 0, up3)
            up1 = self.up_concat1(conv1 * 0, up2)
            up1 = self.dropout2(up1)
