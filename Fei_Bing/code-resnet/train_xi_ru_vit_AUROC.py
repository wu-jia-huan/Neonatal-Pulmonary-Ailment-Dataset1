...
from val_2D import test_single_volume, test_single_volume_ds, test_single_volume_classfier, test_single_volume_classfier_Xi_Ru, test_single_volume_classfier_Fei_Tou,test_single_volume_classfier_Xi_Ru_AUROC
...
parser.add_argument('--num_classes', type=int,  default=5,# 吸入综合征的是二分类
...
def train(args, snapshot_path):
    ...
    for i_batch, sampled_batch in enumerate(testloader):
        metric_i, label_i, prob_i = test_single_volume_classfier_Xi_Ru_AUROC(
            sampled_batch["image"], sampled_batch["label"], model, classes=num_classes)
        metric_list += np.array(metric_i)
        y_true_all.append(label_i)
        y_prob_all.append(prob_i)

    AUROC = roc_auc_score(y_true_all, y_prob_all)

    print("AUROC:", AUROC)
    logging.info("AUROC: %f" % AUROC)
    ...
    writer.add_scalar('info/test_AUROC', AUROC, iter_num)
