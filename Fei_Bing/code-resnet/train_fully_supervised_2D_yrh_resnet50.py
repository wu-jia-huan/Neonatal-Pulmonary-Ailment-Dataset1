...
from val_2D import test_single_volume, test_single_volume_ds, test_single_volume_classfier
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num > 0 and iter_num % 20 == 0:
                model.eval()
                metric_list = 0.0
                metric_list_abs_difference = 0.0
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i, metric_abs_difference = test_single_volume_classfier(
                        sampled_batch["image"], sampled_batch["label"], model, classes=num_classes)
                    metric_list += np.array(metric_i)
                    metric_list_abs_difference += np.array(metric_abs_difference)
                    print("metric_list ", metric_list)
                metric_list = metric_list / len(db_val)
                metric_list_abs_difference = metric_list_abs_difference / len(db_val)
                print("metric_list mean ", metric_list)
                logging.info("metric_list mean : %s", metric_list)
                logging.info("metric_list_abs_difference mean : %s", metric_list_abs_difference)
