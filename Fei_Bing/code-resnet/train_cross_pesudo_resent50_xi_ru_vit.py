...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # print("label_batch[:][:args.labeled_bs]",label_batch[:][:args.labeled_bs].long())
            # print("target",target)

            preds = model1(data)
            pred_x, pred_u_w = preds.split([args.labeled_bs, len(preds) - args.labeled_bs])
            ...
            # pred_u_w,mask_u_w1,mask_u_w2,lam = cutmix(pred_u_w,mask_u_w,alpha)
            # loss_u_w = ce_loss(pred_u_w,mask_u_w1)*lam + ce_loss(pred_u_w,mask_u_w1)
            ...
            loss1 = 0.5 * criteria(outputs1[:], label_batch[:][:].long()) 
            loss2 = 0.5 * criteria(outputs2[:], label_batch[:][:].long()) 
            # print("outputs1[:]",outputs1[:])
            # print("label_batch[:][:].long()",label_batch[:][:].long())

            loss3 = 0.25 * criteria(outputs3[:], target_1[:][:].long()) 
            loss4 = 0.25 * criteria(outputs4[:], target_1[:][:].long()) 
            
            loss5 = 0.25 * criteria(outputs3[:], target_2[:][:].long()) 
            loss6 = 0.25 * criteria(outputs4[:], target_2[:][:].long()) 
            ...
            pseudo_supervision1 = criteria(outputs1[args.labeled_bs:], pseudo_outputs1)
            pseudo_supervision2 = criteria(outputs2[args.labeled_bs:], pseudo_outputs2)
            ...
            pseudo_supervision3 = criteria(outputs3[args.labeled_bs:], pseudo_outputs1)
            pseudo_supervision4 = criteria(outputs4[args.labeled_bs:], pseudo_outputs2)
            loss_sur = +loss3*lam+loss5*(1.-lam)+loss4*lam+loss5*(1.-lam)
