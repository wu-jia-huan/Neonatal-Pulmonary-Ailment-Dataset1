...
from dataloaders.dataset_yrh import BaseDataSets, RandomGenerator, BaseDataSets_xueyoubing, RandomGenerator_single, BaseDataSets_Xi_Ru, BaseDataSets_Fei_Tou, BaseDataSets_Fei_Tou_kfold
...
def run_kfold_training(k=5):
    ...
    五折交叉验证主循环（肺透版）
    """
    base_dir = "/mnt/sdd/wjh/Fei_Bing/code_and_txt_data_process_Fei_Tou"
    save_root = "../model_kfold_Fei_Tou"
    ...
    print("五折交叉验证全部完成！保存目录:", save_root)
...
def train(args, snapshot_path):
    ...
    db_train = BaseDataSets_Fei_Tou_kfold(
    ...
    db_val = BaseDataSets_Fei_Tou_kfold(
        base_dir=args.root_path,
        split="val",
        fold=args.fold
    )

    db_test = BaseDataSets_Fei_Tou_kfold(
        base_dir=args.root_path,
        split="test",
        fold=args.fold
    )
