...
from dataloaders.dataset_yrh import BaseDataSets, RandomGenerator, BaseDataSets_xueyoubing, RandomGenerator_single, BaseDataSets_Xi_Ru_kfold
...
parser.add_argument('--fold', type=int, default=0, help='current fold for k-fold cross-validation')
# label and unlabel
...
def run_kfold_training(k=5):
    ...
    自动：
      ...
      - 调用 train() 函数
    """
    base_dir = "/mnt/sdd/wjh/Fei_Bing/code_and_txt_data_process_Xi_Ru"
    save_root = "../model_kfold"  # 每折模型保存位置
    ...
    for fold in range(k):
        ...
        # 更新每折的txt路径（你的 BaseDataSets_Xi_Ru 会根据 fold 自动读取）
        args.root_path = base_dir
        args.fold = fold   # 需确保 BaseDataSets_Xi_Ru 支持 fold 参数

        # 为每一折创建独立的输出目录
        snapshot_path = os.path.join(save_root, f"fold{fold}")
        os.makedirs(snapshot_path, exist_ok=True)

        # 打印日志信息
        # 移除之前的文件 handler，避免重复打印
        logger = logging.getLogger()
        for h in logger.handlers[:]:
            if isinstance(h, logging.FileHandler):
                logger.removeHandler(h)

        # 为当前 fold 添加新的文件输出 handler
        file_handler = logging.FileHandler(os.path.join(snapshot_path, "log.txt"))
        file_handler.setFormatter(logging.Formatter('[%(asctime)s.%(msecs)03d] %(message)s', '%H:%M:%S'))
        logger.addHandler(file_handler)

        logging.info(f"===== Fold {fold} =====")
        logging.info(str(args))

        # 调用训练函数（这会在内部进行训练+验证+测试）
        result = train(args, snapshot_path)
        logging.info(f"Fold {fold} 完成：{result}\n")

        # 手动清理GPU缓存，避免显存溢出
        torch.cuda.empty_cache()

    print("五折交叉验证全部完成！模型与日志保存在:", save_root)
...
def train(args, snapshot_path):
    ...
    db_train = BaseDataSets_Xi_Ru_kfold(base_dir=args.root_path, split="train", fold=args.fold, num=None,
                                  transform=transforms.Compose([RandomGenerator_single(args.patch_size)]))
    db_val = BaseDataSets_Xi_Ru_kfold(base_dir=args.root_path, split="val", fold=args.fold)
    db_test = BaseDataSets_Xi_Ru_kfold(base_dir=args.root_path, split="test", fold=args.fold)
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            alpha=1.0
            data,target = sampled_batch['image'].to(device), sampled_batch['label'].to(device)
    ...
    preds_dir = os.path.join(snapshot_path, "preds")
    ...
    with torch.no_grad():
        for i_batch, sampled_batch in enumerate(testloader):
            ...
            # 如果是 (H,W) 或 (N,H,W)，转为 (N,1,H,W)
            if imgs.dim() == 3:  
                imgs = imgs.unsqueeze(1)   # 单张图
            elif imgs.dim() == 2:
                imgs = imgs.unsqueeze(0).unsqueeze(0)
            labels = sampled_batch["label"].cpu().numpy()
            outputs = model(imgs)
            probs = torch.softmax(outputs, dim=1).cpu().numpy()
            preds = np.argmax(probs, axis=1)

            for j in range(len(preds)):
                y_true_all.append(int(labels[j]))
                y_pred_all.append(int(preds[j]))
                y_score_all.append(probs[j])
    ...
    np.save(os.path.join(preds_dir, f"y_true_fold{fold}.npy"), y_true_all)
    np.save(os.path.join(preds_dir, f"y_pred_fold{fold}.npy"), y_pred_all)
    np.save(os.path.join(preds_dir, f"y_score_fold{fold}.npy"), y_score_all)
    ...
    TP_total = metric_list[1]
    FN_total = metric_list[2]
    FP_total = metric_list[3]
    metrics_dict = {
        "fold": int(fold),
        "Accuracy": float(Acc),
        "TP": int(TP_total),
        "FN": int(FN_total),
        "FP": int(FP_total),
        "Sensitivity": float(SEN_test),
        "Precision": float(PPV_test),
    }

    metrics_path_json = os.path.join(preds_dir, f"metrics_fold{fold}.json")
    metrics_path_npy = os.path.join(preds_dir, f"metrics_fold{fold}.npy")

    with open(metrics_path_json, "w") as f:
        json.dump(metrics_dict, f, indent=4)
    np.save(metrics_path_npy, metrics_dict)

    print(f"[Fold {fold}] Metrics from test_single_volume_classfier_Xi_Ru saved to {metrics_path_json}")
    logging.info(f"[Fold {fold}] Metrics saved: {metrics_dict}")

    print(f"[Fold {fold}] 预测已保存到: {preds_dir}")
